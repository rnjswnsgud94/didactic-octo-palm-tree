...
export const specialLawLegalSources: LegalSource[] = [
  ...
  },
  {
    ...
    officialUrl:
      "https://www.mcee.go.kr/home/web/board/read.do?boardId=1824110&boardMasterId=39&maxIndexPages=10&maxPageItems=10&menuId=290&pagerOffset=250",
    status: "AUTHORITATIVE",
  },
  {
...
export const specialLawCitations: LegalCitation[] = [
  ...
  },
  {
    ...
    summary: "선택적 사전검토 회의와 조치계획은 각각 20일 이내이며, 정식 경관위원회는 특별한 사유가 없으면 요청일부터 30일 이내 개최한다.",
  },
  {
  ...
  },
  {
    ...
    summary: "민원 처리기간이 5일 이하이면 근무시간으로 계산하고, 6일 이상이면 첫날을 빼고 마지막 날을 포함하되 토요일과 공휴일을 제외한다.",
  },
  {
...
export const specialLawDurations: DurationEstimate[] = [
  {
    ...
    stopClockRules: [
      "민원처리법이 적용되는 관리기관 경로에서는 6일 이상 처리기간의 첫날을 빼고 토요일·공휴일을 제외",
    ...
    sampleSize: null,
    assumptions: [
      "7일은 신청서가 관리기관에 접수된 뒤의 법정 결정기한이며 모집·선정·신청인 준비기간은 별도입니다.",
  ...
  },
  {
    ...
    procedureId: "industrial-complex-occupancy-contract",
    applicantPreparation: null,
    authorityProcessing: { min: 5, base: 5, max: 10, unit: "BUSINESS_DAY" },
    interagencyConsultation: null,
    elapsed: { min: 5, base: 5, max: 10, unit: "BUSINESS_DAY" },
    ...
    sampleSize: null,
    assumptions: [
      "최소·통상 5일은 최초 입주계약의 기본 경로와 변경계약 경로이고, 최대 10일은 최초 계약에 관계기관 인허가 확인·협의가 필요한 분기입니다.",
  ...
  },
  {
    ...
    referencePeriods: [
      ...
      {
        id: "ref-landscape-planning-window",
        kind: "PLANNING_REFERENCE",
        label: "공식 운영자료 기반 1회 심의 계획 참고범위",
        range: { min: 14, base: null, max: 30, unit: "CALENDAR_DAY" },
        jurisdiction: null,
        startsWhen: "안건이 완비되어 해당 회차 접수마감을 충족한 때",
        includes: ["COMMITTEE_WAIT"],
        citationIds: [
          "cit-landscape-guideline-review-timing",
          "cit-goyang-landscape-schedule-duration",
        ...
        observedFrom: null,
        observedTo: null,
        note: "실제 처리실적의 평균·중앙값이 아니라 공식 지침과 2개 지자체 운영일정을 종합한 참고범위입니다. 보완되거나 접수마감을 놓치면 다음 회차로 이월되어 더 길어질 수 있습니다.",
      },
    ],
  },
  {
