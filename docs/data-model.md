...
`planningBasis` states whether the schedule value is a directly applicable official value, an input-resolved branch, an unresolved official branch, an exact local official reference, a cap-only value, a milestone, or insufficient data. Unresolved mutually exclusive branches keep their official range in the detail view but do not insert an arbitrary middle branch into the total. `referencePeriods` preserves additional periods without collapsing their meaning: nationwide statutory/official periods, local service standards, official operating caps, planning references, observed practice, legal deadlines and process milestones. Every reference records its trigger, included components, jurisdiction, citation IDs, sample size and observation window. A local reference is schedule-eligible only when its jurisdiction exactly matches the selected city.
